    ...
    for h_sq in [0.25, 0.5]:
        for rho in [0.0, 0.2, 0.4, 0.5, 0.6, 0.8, 1.0]:
                with open("shell_scripts/temp.sh", "w+") as f:
                    f.write("#!/bin/bash\n")
                    f.write(f"#SBATCH --job-name=AI-TENN_neural_{ddp_str}_batch_norm_h_sq_{h_sq}_rho_{rho}\n")
                    f.write("#SBATCH --mail-user=skumar26@uthsc.edu\n")
                    f.write("#SBATCH --mail-type=BEGIN,END,FAIL\n")
                    f.write("#SBATCH --account=ISAAC-UTHSC0057\n")
                    f.write("#SBATCH --partition=ai-tenn\n")
                    f.write("#SBATCH --qos=ai-tenn\n")
                    ...
                    f.write("#SBATCH --time=02-23:59:59\n")
                    f.write("###########################################\n")
                    ...
                    f.write("source /lustre/isaac/scratch/skumar26/myapps/py_env/tfgpu_217/bin/activate\n")
                    f.write(f"python neural_network_exisitng_Layernorm.py '{ddp_str}' {h_sq} {rho}\n")
                    f.write("echo 'The execution has been done.'\n")
                    f.close()
                with open("shell_scripts/temp.sh", "rb") as fr, open(f"shell_scripts/neural_network_exisitng_{ddp_str}_batch_norm_h_sq_{h_sq}_rho_{rho}_h100.sh", "wb") as fw:
                    for l in fr.readlines():
                        fw.write(l.replace(b"\r\n",b"\n"))
                    f.close()

                # append a line to a .txt file and create if it doesn't exist
                with open("shell_scripts/sbatch_commands_scripts_h100.txt", "a+") as f:
                    f.write(f"sbatch neural_network_exisitng_{ddp_str}_batch_norm_h_sq_{h_sq}_rho_{rho}_h100.sh\n")
                    f.close()    
